...
from opengnn.models.graph_cnn_to_sequence import GraphConvToSequence

from opengnn.encoders import GGNNEncoder, SequencedGraphEncoder
from opengnn.encoders.graph_conv_encoder import GraphConvEncoder
...
from opengnn.utils.attention_wrapper import BahdanauAttention, LuongAttention
        ...
        for train_step in range(initial_step+1, args.train_steps+1):
            ...
            if train_step % args.validation_interval == 0:
                valid_loss, valid_rouge, rouge_2, rouge_lcs = evaluate(
                    ...
                    predictions,
                    valid_targets)
                print("eval loss: %0.4f, eval rouge: %0.4f, eval rouge_lcs: %0.4f, eval rouge_2: %0.4f" % (valid_loss, valid_rouge, rouge_lcs, rouge_2))
...
def evaluate(session,
    ...
    rouge, rouge_lcs, rouge_2 = compute_rouge(valid_predictions, targets)
        ...
        model = GraphConvToSequence(
            ...
            encoder=GraphConvEncoder(
                ...
                base_cnn_encoder=CNNEncoder(
                    node_feature_size=args.node_features_size,
                    num_kernel_1=args.node_features_size,
                    num_kernel_2=args.node_features_size,
                    kernel_size_1=4,
                    kernel_size_2=4),
...
def compute_rouge(predictions, targets):
    ...
    predictions = [prediction if len(prediction)>0 else "EMPTY" for prediction in predictions]
    targets = [" ".join(target).lower().strip().replace('.', 'EMPTY') for target in targets]
    targets = [target if len(target)>0 else "EMPTY" for target in targets]
    rouge = Rouge()
    try:
        ...
        print('value error')
        return 0.0
    return scores['rouge-1']['f'], scores['rouge-l']['f'], scores['rouge-2']['f']
