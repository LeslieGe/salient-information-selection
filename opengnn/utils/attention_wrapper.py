...
from opengnn.utils.ops import batch_unsrt_segment_sum
        ...
        with variable_scope.variable_scope(None, "luong_attention", [query]):
            ...
            if self._scale:
                attention_g = variable_scope.get_variable(
                    ...
                    shape=())
            attention_conv = variable_scope.get_variable(
                name='attention_conv',
                shape=(4, 256, 1),
                initializer=tf.truncated_normal_initializer(stddev=0.1))
            score = _luong_score(query, self._keys, attention_g)
        
        #p_gen = tf.sigmoid(self.p_gen_linear(query))
        #score = p_gen * score
        alignments = self._probability_fn(score, state)
        #alignments = score

        batch_size = tf.shape(alignments, out_type=tf.int64)[0]
        max_length = tf.shape(alignments, out_type=tf.int64)[1]

        #extend_v = tf.nn.l2_normalize(
        #    self._extend_memory * tf.reshape(query, [-1, 1, self._num_units]), axis=2)
        extend_v = tf.nn.l2_normalize(self._extend_memory, axis=-1) * tf.reshape(query, [-1, 1, self._num_units])
        extend_attention = tf.nn.conv1d(extend_v, filters=attention_conv, stride=1, padding="SAME")
        extend_attention = tf.squeeze(extend_attention, axis=2)
        #extend_attention = (1 - p_gen) * extend_attention
        #extend_attention = tf.nn.l2_normalize(extend_attention, axis=-1)
        #extend_attention = tf.nn.softmax(extend_attention, axis=-1)
        extend_mask_attention = extend_attention * tf.sequence_mask(self._extend_memory_length, dtype=tf.float32)
        #extend_attention = (1 - p_gen) * extend_attention        

        alignments = batch_unsrt_segment_sum(
            data=tf.concat([alignments, extend_mask_attention], axis=-1),
            segment_ids=tf.concat(
                [tf.tile(tf.reshape(tf.range(max_length), (1, -1)), [batch_size, 1]),
                 self._extend_memory_sequence], axis=-1),
...
class BahdanauAttention(_BaseAttentionMechanism):
    ...
    def __call__(self, query, state):
        ...
        #extend_v = tf.nn.l2_normalize(
        #    self._extend_memory * tf.reshape(query, [-1, 1, self._num_units]), axis=2)
        #extend_v = self._extend_memory * tf.reshape(query, [-1, 1, self._num_units])
        #extend_attention = tf.nn.l2_normalize(
        #    tf.squeeze(tf.nn.conv1d(extend_v, filters=attention_conv, stride=1, padding="SAME"), axis=2), axis=1)
        #extend_attention = tf.nn.softmax(extend_attention, axis=-1)
        #extend_mask_attention = extend_attention * tf.sequence_mask(self._extend_memory_length, dtype=tf.float32)
...
def _compute_attention(attention_mechanism, cell_output, attention_state,
    ...
    #expanded_alignments = array_ops.expand_dims(tf.nn.softmax(alignments,axis=-1), 1)
...
class AttentionWrapper(rnn_cell_impl.RNNCell):
    ...
    def call(self, inputs, state):
        ...
        if self._output_attention:
            return attention, next_state, context_, alignments
        else:
            return cell_output, next_state, context_, alignments
