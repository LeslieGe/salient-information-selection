...
from opengnn.utils.ops import batch_unsrt_segment_logsumexp, batch_unsrt_segment_sum
...
def unnormalized_luong_attention(*args):
    #return LuongAttention(*args)
    return LuongAttention(*args, probability_fn=lambda scores: scores)
    #return BahdanauAttention(*args, probability_fn=lambda scores: scores)
...
class CopyingWrapper(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self,
        ...
        self.concat_linear = tf.layers.Dense(256, use_bias=False)
    ...
    def call(self, inputs, state):
        ...
        cell_output, next_cell_state, context_, copying_alignments = self._cell(inputs, cell_state)
        #cell_output = self.concat_linear(tf.concat([cell_output, context_], axis=-1))
        ...
        #base_logits = tf.nn.softmax(base_logits, axis=-1)
        ...
        copying_logits = (1 - p_gen) * copying_logits
